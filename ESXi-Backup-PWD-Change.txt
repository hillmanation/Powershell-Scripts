...
ForEach ($ESXiHost in $hosts) {
    ...
    ## Connect to each host and update the recovery password to the newly entered one
    Connect-VIServer -server $esxihost -Credential $CurrentCreds -NotDefault
    Set-VMHostAccount -server $esxihost -UserAccount recovery -Password $NewCreds.GetNetworkCredential().Password
}
