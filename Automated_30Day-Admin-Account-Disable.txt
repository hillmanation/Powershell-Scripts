...
$users = Get-ADuser -filter * -Properties Name,samAccountName,DistinguishedName,LastLogonDate,Created | where { $_.Enabled -eq $True -and $_.samAccountName -like "zz*" -and $_.LastLogonDate -ne $NULL -and (New-TimeSpan -start $_.LastLogonDate -end $date).Days -gt 30 -and (New-TimeSpan -start $_.Created -end $date).Days -gt 30 } | Select-Object Name,samAccountName,DistinguishedName,LastLogonDate,Created
...
$adminfolder = $outfolder + "\" + "Admins\"

If (!(Test-Path -Path $adminfolder)) { New-Item -Path $outfolder -Name "Admins" -ItemType Directory -Force | Out-Null }

$fileNameCSV = $adminFolder + "AD_Admin_Disabled_Audit_" + $fullmonth + ".csv"
$fileNameLog = $adminFolder + "AD_Admin_Disabled_Log_" + $filedate + ".txt"

## Start text log
"--------------------$date--------------------`r`n---------Starting AD Admin Inactivity Audit Script---------`r`n-----------------------------------------------------------`r`n`r`n" | Out-File $filenameLog -Append
...
If ($users -ne $NULL) {
    ForEach ($user in $users) { 
        ...
        "Account $samaccountname for Admin $name last logged in $lastlogon, created on $created...`r`n`r`nDisabling Admin account..." | Out-File $filenameLog -Append
        ...
        If ((Get-AdUser -identity $samaccountname -Properties Enabled | Select-Object Enabled).Enabled -ne $False) { 
            "---ADMIN ACCOUNT WAS NOT DISABLED, this could be due to a permissions error, please investigate---`r`n`r`n" | Out-File $filenameLog -Append
            Continue
        }

        "Account disabled, generating new description and noting new disabled on date..." | Out-File $filenameLog -Append

        $newdescr = "Disabled over 30 days inactivity via automated script $descrdate"
        ...
        $newdescr = "Disabled over 30 days inactivity via automated script $descrdate - " + $descr
        ...
        $groups | % { $grouplist += $_ + ";" }
        $output = [pscustomobject]@{ samAccountName = $samaccountname; Name = $user.Name; DateDisabled = $descrdate; Reason = '30 Day Inactivity'; OrganizationalUnit = $OU; GroupsRemoved = 'False'; Memberof = $grouplist }
        $output | Export-CSV -Path $filenameCSV -NoTypeInformation -Append

        "Admin account for $name has been successfully disabled and audit logs created.`r`nMoving to next user...`r`n`r`n" | Out-File $filenameLog -Append
...
"--------AD Admin Inactivity Audit Script Completed---------`r`n--------------------$end--------------------`r`n--------------Elapsed time: $elapsed seconds--------------`r`n`r`n" | Out-File $filenameLog -Append